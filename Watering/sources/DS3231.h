...
public:
	...
	// Time functions
	void get_time(byte &year, byte &month, byte &date, byte &week,
	...
	void set_week(byte week);
	void set_hour(byte hour, bool h12);
	...
	// Alarm functions(day = dy ? week : date)
	...
	bool is_alarming(byte alarm, bool clear);

	// Oscillator functions(Attention to use!!!)
	void enable_oscillator(bool enable, bool battery, byte frequency);
	void enable_32kHz_output(bool enable);
	bool is_oscillator_running();

protected:
	byte _dec_to_bcd(byte val);
	byte _bcd_to_dec(byte val);
	byte _read_control_byte(byte which);
	void _write_control_byte(byte which, byte val);
};
